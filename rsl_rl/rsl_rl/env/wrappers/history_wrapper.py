...
class HistoryWrapper(gym.Wrapper):
    ...
    def step_learn(self, action_student,student_teacher):
        # privileged information is concatenated to the observation, and observation history is stored in info
        obs, privileged_obs, rew, done, info, teacher_action = self.env.steplearn(action_student,student_teacher)
        # privileged_obs = info["privileged_obs"]

        self.obs_history = torch.cat((self.obs_history[:, self.env.num_obs:], obs), dim=-1)
        return {'obs': obs, 'privileged_obs': privileged_obs, 'obs_history': self.obs_history}, rew, done, info,teacher_action
    ...
    def get_reward_buf(self):
        return self.env.get_reward_buf()
